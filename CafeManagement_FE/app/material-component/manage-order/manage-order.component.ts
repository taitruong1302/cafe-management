...
export class ManageOrderComponent implements OnInit {
  ...
  dataSource: any = [];
  manageOrderForm: FormGroup = new FormGroup({
  ...
  getProductsByCategory(value: any) {
    this.productService.getProductsByCategory(value.id).subscribe((res: any) => {
      this.products = res;
      this.getControl('price')?.setValue('');
      this.getControl('quantity')?.setValue('');
      this.getControl('total')?.setValue(0);
    }, error => {
      this.ngxService.stop();
      console.log(error.error?.message);
      if (error.error?.message) {
  ...
  getProductDetails(value: any) {
    this.productService.getById(value.id).subscribe((res: any) => {
      ...
      this.getControl('price')?.setValue(res.price);
      this.getControl('quantity')?.setValue('1');
      this.getControl('total')?.setValue(this.price * 1);
    }, error => {
      this.ngxService.stop();
      console.log(error.error?.message);
      if (error.error?.message) {
  ...
  setQuantity(value: any) {
    var temp = this.getControl('quantity')?.value;
    if (temp > 0) {
      this.getControl('total')?.setValue(this.getControl('quantity')?.value * this.getControl('price')?.value)
    }
    else if (temp != '') {
      this.getControl('quantity')?.setValue('1');
      this.getControl('total')?.setValue(this.getControl('quantity')?.value * this.getControl('price')?.value)
  ...
  validateProductAdd() {
    if (this.getControl('total')?.value === 0 || this.getControl('total')?.value === null || this.getControl('quantity')?.value <= 0) {
      return true;
    }
    else {
      return false;
  ...
  validateSubmit() {
    if (this.totalAmount === 0 || this.getControl('name')?.value === null || this.getControl('email')?.value === null || this.getControl('contactNumber')?.value === null || this.getControl('paymentMethod')?.value === null) {
      return true;
    }
    else {
      return false;
  ...
  add() {
    ...
    var productName = this.dataSource.find((e: { id: any }) => e.id === formData.products.id);
